...
```{r}
path <- "Barak/raw data files/nowcasting_data_raw.xlsx"
```
...
```{r}
save_list_to_excel(blocks_real, "Barak/transformation data/blocks_real.xlsx")
```
...
```{r}
td <- read_csv("Barak/data_for_sa/td_var.csv")
td_ts <- ts(
  td[, -1],
...
preadj <- read_excel("Barak/data_for_sa/hol_preadj.xlsx") %>%
...
```{r}
write_sa_info(sa_info, "Barak/transformation data/SA_results_01.xlsx")
```

```{r}
save_list_to_excel(blocks_sa, "Barak/transformation data/blocks_sa_01.xlsx")
```
...
apply on each block and save the recommendations in a named list. Later we will apply transformations with the recommended codes.
...
The chunk below converts the test recommendations into numeric transformation codes, applies them to every model input block, and stores the transformed data in `blocks_transformed`.
...
test for the results of the transformations
...
```{r}
save_transformation_info(transformation_info, "Barak/transformation data/TRANSFORMATION_INFO.xlsx")

```

```{r}
save_list_to_excel(blocks_transformed, "Barak/transformation data/blocks_transformed.xlsx")
```
...
```{r}
shift_block_by_named_lags <- function(df, lags) {
  ...
  for (var in vars) {
    ...
    if (!var %in% names(lags)) {
      warning(paste("No lag defined for variable:", var, "- using 0"))
      k <- 0
    } else {
      k <- lags[[var]]
    }
    ...
    if (k == 0) {
      ...
      df_shifted[[var]] <- dplyr::lag(df[[var]], n = k)
    }
  }
  
  report <- data.frame(
    variable = vars,
    lag_months = sapply(vars, function(v) {
      if (v %in% names(lags)) lags[[v]] else 0
    })
  )
...
#   "REER" = 1,
#   "Dollar" = 0,
#   "Foreign exchange reserves (millions of dollars)" = 1
# )
```
...
release_lags <- list(
  ...
  personal_labor_income_taxes = 1,
  corporate_business_tax      = 1,
  consumption_tax             = 1,
  import_trade_tax            = 1,
  real_estate                 = 1,
  real_activity               = 2,
...
for (sheet_name in names(release_lags)) {
  
  if (!sheet_name %in% names(blocks_transformed)) {
    warning(paste("Sheet not found in blocks_transformed:", sheet_name))
    next
  }
  
  block <- blocks_transformed[[sheet_name]]
  vars <- setdiff(names(block), "Date")
  lag_def <- release_lags[[sheet_name]]
  
  # Case 1: one lag for the whole sheet
  if (length(lag_def) == 1 && is.null(names(lag_def))) {
    
    lags <- setNames(
      rep(lag_def, length(vars)),
      vars
    )
    
  # Case 2: different lag for each variable
  } else {
    
    lags <- lag_def
    
    missing_vars <- setdiff(vars, names(lags))
    
    if (length(missing_vars) > 0) {
      warning(
        paste(
          "Missing lag definition in sheet",
          sheet_name,
          "for variables:",
      ...
      lags <- c(
        lags,
        setNames(rep(0, length(missing_vars)), missing_vars)
      )
    }
  }
  
  out <- shift_block_by_named_lags(
    df = block,
    lags = lags
  )
  
  blocks_shifted[[sheet_name]] <- out$data
  
  shift_report[[sheet_name]] <- out$report |>
    dplyr::mutate(sheet = sheet_name)
}

shift_report <- dplyr::bind_rows(shift_report)
```

### Save Results

```{r}
blocks_shifted_path <- "Barak/transformation data/blocks_shifted.xlsx"

save_list_to_excel(blocks_shifted, blocks_shifted_path)
```

```{r}
save_transformation_info(shift_report, "Barak/transformation data/shift_report.xlsx")
```

# Create Unified Table for Modeling

this script takes your transformed data and turn it into unified sheet that fits the format of DFM.qmd
...
output_path <- "Barak/transformation data/combined_monthly_panel_Q_refined.xlsx"
...
# Target dates are end-of-month; move them to first day of next month
    ...
    # }
    if (sheet_name == "target") {
      df$Date <- lubridate::floor_date(df$Date %m+% months(1), "month")
    }
...
\`\`\`{r} td \<- read_csv("../PROJ_DFM/data_for_sa/td_var.csv") td_ts \<- ts( td\[, -1\], start = c(year(min(td$date)), month(min(td$date))), frequency = 12 )

preadj \<- read_excel("../PROJ_DFM/data_for_sa/hol_preadj.xlsx") %\>% mutate(date = as.Date(date)) \# ...existing code... \`\`\`{r} \# Input: multi-sheet transformed + lag-adjusted workbook if (!exists("blocks_shifted_path")) { blocks_shifted_path \<- "../PROJ_DFM/lag_adjusted/blocks_shifted.xlsx" } \# ...existing code...